...
contract MerkleRelay is MerkleRelayCore {
    ...
    mapping(address => bytes32[]) rootsSubmittedByClient;
    mapping(address => uint) clientStake;

    event NewRoot(bytes32 root);


    constructor(bytes memory _rlpHeader, uint totalDifficulty, address _ethashContractAddr) MerkleRelayCore(_rlpHeader, totalDifficulty, _ethashContractAddr) {}
    ...
    function getRequiredStakePerRoot() public pure returns (uint) {
    ...
    function getBlockHashesSubmittedByClient() public view returns (bytes32[] memory) {
        return rootsSubmittedByClient[msg.sender];
    }

    function submitRoot(bytes[] calldata headers, bytes32 parentRoot) public {
        ...
        // client has enough stake -> submit headers and add its hash to the client's list of submitted block headers
        bytes32 newRootHash = _submitRoot(headers, parentRoot);

        // POST ACTION
        rootsSubmittedByClient[msg.sender].push(newRootHash);
        emit NewRoot(newRootHash);
    }

    function disputeBlockHeader(bytes calldata rlpHeader, TreeProof calldata proofMeta, bytes calldata rlpParent, TreeProof calldata proofParentMeta, bytes32 root, bytes32 rootParent, PoWMetadata calldata powmetadata) public {
        ...
        require(_verifyBlock(proofParentMeta, rlpParent, rootParent), "parent block not in root");
    ...
    ///      submitted block headers (rootsSubmittedByClient). It does not matter whether a block's lock period has already
    ///      been elapsed. As long as the block is referenced in rootsSubmittedByClient, the stake is considered as "used".
    function getUnusedStake(address client) private view returns (uint) {
        uint usedStake = rootsSubmittedByClient[client].length * REQUIRED_STAKE_PER_BLOCK;

        if (clientStake[client] < usedStake) {
            // if a client get punished due to a dispute the clientStake[client] can be less than
            // rootsSubmittedByClient[client].length * REQUIRED_STAKE_PER_BLOCK, since clientStake[client] is deducted
            // after the dispute, but rootsSubmittedByClient[client] remains unaffected (i.e., it is not cleared)
            return 0;
        } else {
    ...
    /// @dev Checks for each block referenced in rootsSubmittedByClient whether it is unlocked. In case a referenced
    ///      block's lock period has expired, its reference is removed from the list rootsSubmittedByClient.
    function cleanSubmitList(address client) private returns (uint) {
        ...
        // rootsSubmittedByClient-list, in this case it can happen, that a client tries to
        ...
        // two described cases
        for (uint i = 0; i < rootsSubmittedByClient[client].length;) {
            bytes32 blockHash = rootsSubmittedByClient[client][i];

            if (!isRootStored(blockHash) || isUnlocked(blockHash)) {
                // block has been removed or is already unlocked (i.e., lock period has elapsed) -> remove hash from array
                uint lastElemPos = rootsSubmittedByClient[client].length - 1;
                // copy last element to position i (overwrite current elem)
                rootsSubmittedByClient[client][i] = rootsSubmittedByClient[client][lastElemPos];
                // remove last element
                rootsSubmittedByClient[client].pop();
                deletedElements += 1;
